import { Activity, AlertTriangle, Box, Clock, Globe, Server, ShieldAlert } from "lucide-react";
...
export function SummaryCards({ summary }: { summary: Summary }) {
  const cards = [
    ...
    {
      label: "Last scan",
      value: summary.last_scan_time ? formatter.format(new Date(summary.last_scan_time)) : "Never",
      icon: Clock
    }
  ];
      ...
      {cards.map((card) => {
        ...
        return (
          ...
          >
            ...
            <p className="mt-3 truncate text-2xl font-semibold text-ink">{card.value}</p>
          </section>
        );
      })}
    </div>
